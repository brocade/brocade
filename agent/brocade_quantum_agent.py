#!/usr/bin/env python
#
# Derived from Linux Bridge Plugin Agent
#
import logging
import os
import shlex
...
class LinuxBridge:
    ...
    def add_interface(self, network_id, physical_interface, vlan_id,
                      port_id):
        if not port_id:
            """
        ...
        if physical_interface.startswith(GATEWAY_INTERFACE_PREFIX):
...
class LinuxBridgeRpcCallbacks():
    ...
    def port_update(self, context, **kwargs):
        ...
        physical_interface = kwargs.get('physical_interface')
        if port['admin_state_up']:
            vlan_id = kwargs.get('vlan_id')
            # create the networking for the port
            self.linux_br.add_interface(port['network_id'], physical_interface,
                                        vlan_id,
                                        port['id'])
        else:
...
class LinuxBridgeQuantumAgentRPC:
    ...
    def daemon_loop(self):
        ...
        while True:
            start = time.time()
            i = i + 1
            LOG.warning("Loop iteration %d", i)
            if sync:
